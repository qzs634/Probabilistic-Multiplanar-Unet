...
class ImageLoader():
    ...
    def get_pair(self, index=None):
        ...
        try:
            image = nib.load(os.path.join(self.image_path, "image" + str(index) + ".nii"))
            label = nib.load(os.path.join(self.label_path, "label" + str(index) + ".nii"))
            self.index += 1
    ...
    def get_random_pair(self):
        i = random.randint(0, self.file_count)
