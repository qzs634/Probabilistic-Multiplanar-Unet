...
def dice(pred, truth):
    dice = dice_coeff((pred > 0.5).float(), truth).item()
    return dice
...
|  $$$$$$/| $$ \  $$| $$$$$$$$   | $$         | $$$$$$$$   \  $/   | $$  | $$| $$$$$$$$|  $$$$$$/| $$  | $$   | $$    /$$$$$$|  $$$$$$/| $$ \  $$
 \______/ |__/  \__/|________/   |__/         |________/    \_/    |__/  |__/|________/ \______/ |__/  |__/   |__/   |______/ \______/ |__/  \__/
    ...
    train = UNetTrainer(device, n_channels=1, n_classes=1, load_model=args.load)
    ...
    with tqdm(total=N, desc=f'Predictions ', unit='img') as pbar:
        for data in loader:
            ...
            if slice_count == n_slices:
                ...
                vol_1_dice += dice(volume1, true_mask)
                ...
                dices = dice(avg_volume, true_mask)

                dice_sums += dices

                if best_dice is None or dices > best_dice and torch.max(avg_volume) > 0:
                    best_dice = dices
                    best_volume = avg_volume
                    true_volume = true_mask
                    best_index = img_count
                    logging.info(f'Best index {best_index} = {best_dice}')
        ...
        print("view 1 dice: ", vol_1_dice / len(dataset.ids))
        print("view 2 dice: ", vol_2_dice / len(dataset.ids))
        print("view 3 dice: ", vol_3_dice / len(dataset.ids))
