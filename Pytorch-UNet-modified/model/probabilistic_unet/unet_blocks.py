...
class UpConvBlock(nn.Module):
    ...
    def forward(self, x, bridge):
        ...
        if not up.shape[3] == bridge.shape[3]:
            diffY = torch.tensor([bridge.size()[2] - x.size()[2]])
            diffX = torch.tensor([bridge.size()[3] - x.size()[3]])

            up = F.pad(x, [diffX // 2, diffX - diffX // 2, diffY // 2, diffY - diffY // 2])
